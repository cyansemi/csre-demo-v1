                    ...
                    uuid_t & xclbinId)
{
    ...
    csre_config.CSRE_MAX_CHAR_NUM       = xclbinGetKeyValue (bit, "CSRE_MAX_CHAR_NUM");
    csre_config.CSRE_MAX_STATE_NUM      = xclbinGetKeyValue (bit, "CSRE_MAX_STATE_NUM");
    csre_config.CSRE_MAX_CHAR_PER_TOKEN = xclbinGetKeyValue (bit, "CSRE_MAX_CHAR_PER_TOKEN");
    csre_config.CSRE_MAX_TOKEN_NUM      = xclbinGetKeyValue (bit, "CSRE_MAX_TOKEN_NUM");
    csre_config.CSRE_MAX_COUNT_NUM      = xclbinGetKeyValue (bit, "CSRE_MAX_COUNT_NUM");

    if (update_csre_regex_configs (csre_config)) {
        throw std::runtime_error ("Failed to set csre configurations");
        return -1;
    }
    ...
    if (max_cu_index > layout->m_count) {
        delete [] header;
...
{
    if (handle == XRT_NULL_BO) {
        throw std::system_error (errno, std::generic_category(), "buffer object allocation");
    }
}
